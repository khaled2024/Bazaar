...
final class SearchController: UIViewController {
    ...
    private func collectionCellRegister() {
        searchView.searchCollection.register(ProductCollectionViewCell.self, forCellWithReuseIdentifier: ProductCollectionViewCell.identifier)
    }
...
extension SearchController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = searchView.searchCollection.dequeueReusableCell(withReuseIdentifier: ProductCollectionViewCell.identifier, for: indexPath) as? ProductCollectionViewCell else { return UICollectionViewCell()}
        cell.interface = self
        if isSearchBarEmpty {
            cell.configCell(searchViewModel.products[indexPath.row])
            return cell
        } else {
            cell.configCell(filteredProducts[indexPath.row])
            return cell
        }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return CGSize(width: searchView.searchCollection.frame.width / 2 - 10, height: searchView.searchCollection.frame.width / 1.5 )
...
extension SearchController: ProductsCollectionViewCellInterface {
