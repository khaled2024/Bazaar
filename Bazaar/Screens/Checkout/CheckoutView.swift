...
final class CheckoutView: UIView {
    ...
    private var imageView = CustomImageView(image: UIImage(named: "cart"), tintColor: .clear, contentMode: .scaleAspectFit)
    private var viewForImage = CustomView(backgroundColor: .clear, cornerRadius: 75)
    private var label1 = CustomLabel(text: "Order Successful!", numberOfLines: 0, font: FiraSana.bold.rawValue, size: 18, textColor: .label, textAlignment: .center)
    private var label2 = CustomLabel(text: "You have successfully made order", numberOfLines: 0, font: FiraSana.bold.rawValue, size: 14, textColor: .label, textAlignment: .center)
    ...
    func setCheckoutModel() {
        checkoutModel = [CheckoutModel(image: UIImage(named: "cart")!, label1: "Your cart is currently empty", label2: "You can checkout when there is a product in your cart.")]
    }
